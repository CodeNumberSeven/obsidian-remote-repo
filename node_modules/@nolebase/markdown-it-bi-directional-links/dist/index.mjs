import { sep, relative, basename, extname, dirname, posix } from 'node:path';
    ...
    md.inline.ruler.after("text", "bi_directional_link_replace", (state) => {
      ...
      if (isRelativePath) {
        if (state.env.relativePath) {
          resolvedNewHref = relative(dirname(state.env.relativePath), matchedHref).split(sep).join("/");
        } else if (state.env.filePathRelative) {
          resolvedNewHref = relative(dirname(state.env.filePathRelative), matchedHref).split(sep).join("/");
        } else {
